...
func (h *Handler) listMyAppeals(c *gin.Context) {
	...
	if ticketID != "" {
		// Если указан ticket_id, возвращаем обжалования по тикету
		appeals, err := h.appealService.ListByTicketID(c.Request.Context(), principal, ticketID)
		if err != nil {
			h.handleError(c, err)
			return
		}
	...
	// Если ticket_id не указан, возвращаем все обжалования водителя
	// (это можно реализовать через отдельный метод в сервисе, но пока используем существующий)
	c.JSON(http.StatusBadRequest, errorResponse("ticket_id is required for now"))
}
